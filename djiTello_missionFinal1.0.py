...
def pos_correction_withYaw(frame_centre, marker_centre_x, marker_centre_y, fb_pos, yaw_pos, p_lr_error, p_fb_error, p_ud_error, p_yaw_error, currentTime, previousTime, cI_ud, cI_fb):
    ...
    if 40<= fb_pos <=80 and 450<= marker_centre_x <=510 and yaw_ab_pos >0:
        kp_lr = 1
        kd_lr = 0.1
        lr_error = yaw_pos
...
#------WAYPOINT MARKERS------#
move_right_markers = [1,2]
move_up_markers = [0]
move_left_markers = [4,5]
move_down_markers = [3]
#----------------------------#
...
#----VIDEO WRITER----#
#out = cv.VideoWriter('output3.avi',cv.VideoWriter_fourcc('M','J','P','G'), 30, (960,720))
#-----------#
...
while True:
    ...
    try:
        ...
        cv.putText(frame,f"Yaw:{int(math.degrees(yaw_marker))}",(10,80),cv.FONT_HERSHEY_PLAIN,1.3,(255,0,0),1,cv.LINE_AA)
    ...
    for ids, corners in zip(itertools.repeat(marker_IDs), marker_corners):
        ...
        if ids[0][0] == 3:
            cv.putText(frame,f"EMPTY BIN",top_left,cv.FONT_HERSHEY_PLAIN,3,(0,255,0),3,cv.LINE_AA)
        ...
        try:
            ...
            if 0<=abs(yaw_pos)<=3 and frame_centre[0]-30 <= marker_centre_x <= frame_centre[0]+30 and 65<= fb_pos <=75 and frame_centre[1]-50 <= marker_centre_y <= frame_centre[1]+50:
                tello.send_rc_control(0,0,0,0)
                            
                if var == var1:
                    if var in move_right_markers:
                        tello.move_right(123)
                        var1 += 1
                        print("Tello Moving Right!")
                        print("var1",var1)
                    
                    
                    elif var in move_up_markers:
                        tello.move_up(98)
                        var1 += 1
                        print("Tello Moving Up!")
                        print("var1",var1)

                    elif var in move_left_markers:
                        tello.move_left(230)
                        var1 = 0
                        print("Tello Moving Left")
                        print("var1",var1)

                    elif var in move_down_markers:
                        tello.move_down(98)
                        var1 += 1
                        print("Tello Moving Left")
                        print("var1",var1)
    ...
    for code_data in code:
        ...
        mydata= code_data.data.decode('utf-8')
        print(type(mydata))
        pts = np.array([code_data.polygon],np.int32)
        ...
        pts2_y=int(pts2[1])
        print(pts2[0], pts2[1])
        cv.putText(frame,mydata,(pts2_x,pts2_y),cv.FONT_HERSHEY_PLAIN,1.3,(0,0,255),1,cv.LINE_AA)
    ...
    try:
        ...
        data = {"drone_status": "Online" , "drone_battery": drone_battery, "drone_current_location" : dcl, "drone_current_location_id": dclid, "package_id": pack_id, "package_count": pack_count}
    ...
    try:
        ...
        producer.send("DroneStream", buffer.tobytes())
    except:
        pass
    #-------------------------------------------------------#
